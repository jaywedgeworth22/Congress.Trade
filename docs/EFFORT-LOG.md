...
## Deployed
- **Premium = delivery only: un-gate all feed columns (CLAUDE, AG, owner-directed 2026-07-17) — PR MERGED / DEPLOYED / LIVE VERIFIED 2026-07-17 via PR #522.** Owner decision: "cap and country and no column should be premium — only delivery via webhook and SSE." Removed `tier: 'premium'` from the Cap, Country, and Sector column defs in both the legacy HTML dashboard and Next.js PWA, removed the locked-column chooser branch, the gated CTA, and the "Premium enrichment" notes/upsell. The `sector` column is hidden by default for all users via `def: false` in its column definition; `loadHiddenCols` returns an existing `feed-cols-hidden-v3` array unchanged (no migration), so existing non-premium users with a pre-upgrade saved array (which lacked `sector` because it was formerly premium-only) see Sector visible after upgrading rather than hidden by a migration step. Merged to `main` via PR #522 (all tests passed) and deployed to production via `ship.sh` with live health check verification.
- **Benchmark Run Models Dynamic Validation & OpenRouter Pricing (AG) — DEPLOYED / LIVE VERIFIED 2026-07-17 via PR #546 & PR #547.** Validates benchmark run candidates against a static catalog assembled from `DEFAULT_CANDIDATES`, `LLAMAPARSE_CANDIDATES`, and `LEGACY_CANDIDATES` arrays (`app/src/benchmark/settings.ts`) plus the hardcoded OpenRouter candidates in `app/src/extraction/bakeoff.ts` — the model list is not dynamically sourced from the live OpenRouter API. Adds rate cards for `openai/gpt-5.6-terra-pro` and `anthropic/claude-haiku-4.5`, maps nested OpenRouter provider/model combos to their base pricing in `findRate`, resolves the OpenAI model-access 500 fallback error, parses PDF page counts natively using `unpdf`'s `getDocumentProxy` in `OpenRouterVisionExtractor`, and returns/propagates `pageCount`/`pagesProcessed` in the extractor results. PR #546 and PR #547 squash-merged to `main` (all typechecks and unit tests passed) and deployed to production via `ship.sh` with live health check verification.
