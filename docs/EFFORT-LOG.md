...
- **Telemetry half-open single-probe circuit breaker hardening (CODEX + 3-agent review team, owner-directed, M) — PR #587 MERGED INCOMPLETE / CORRECTIVE GATES GREEN / FOLLOW-UP IN PROGRESS 2026-07-18.** PR #587 squash-merged as `7389e09`, but its remote conflict-resolution merge omitted the reviewed core `thirdPartyTelemetry.ts` and regression-test changes while landing the migration/index/readiness/type support. Follow-up branch `codex/pr587-core-followup` starts from exact merged `main`, preserves every unexpected remote/main commit, and reconciles the omitted #587 core with #584's newer terminal-event queue boundary. It also closes review-discovered loss/coordination gaps: capacity cannot ACK when exact-event R2 durability is required, Infisical resolution completes before the singleton half-open lease is claimed, 401 remains transient and opens the outage circuit, and malformed R2 bytes are durably quarantined by source-object identity before deletion (retained when quarantine fails). Intended behavior: ACK Queue/DLQ only after receiver acceptance, a deterministic event reject, or exact-event R2 durability; quarantine terminal/malformed R2 poison objects; bound headers plus response-body parsing; fail closed on unavailable/malformed breaker control; persist a KV probe-in-flight gate behind atomic D1 lease 0046; prevent stale contenders from reopening after a successful probe; and retain transient/auth/rate-limit/5xx/timeout/malformed-2xx failures. Verification: focused telemetry/queue/readiness/migration 89/89, TypeScript, changed-file ESLint (0 errors), diff check, and serialized full backend 143 files / 1,500 tests all pass. KEEPOUT remains until the follow-up and the independent current-main PWA repair are green/merged, then canonical deploy and live verification complete.
