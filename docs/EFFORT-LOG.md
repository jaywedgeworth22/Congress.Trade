...
- **Telemetry half-open single-probe circuit breaker hardening (CODEX, owner-directed, M) — IN PROGRESS 2026-07-18.** Follow-up to merged PR #552 and conflicted PR #584 on branch `codex/telemetry-half-open-lease` / worktree `/Users/jay/apps/congress-telemetry-circuit-hardening`. Carry forward the R2-only outbox and review fixes, add a bounded receiver timeout plus an atomic singleton half-open probe lease that fails closed when its durable coordinator is unavailable, and verify concurrency/recovery before landing and production deployment. KEEPOUT: `app/src/shared/thirdPartyTelemetry.ts`, related types/tests, and the new lease migration until handoff.
