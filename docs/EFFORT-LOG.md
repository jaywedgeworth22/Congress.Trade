...
## In Progress
- **Dashboard title-only stale-PR carry-forward (CODEX + verifier, S) — IMPLEMENTED / LOCAL GATES GREEN 2026-07-13.** On `codex/dashboard-title-only`, the production dashboard uses `<title>Congress.Trade</title>` and the historical mock uses `<title>Congress.Trade (Design Mock)</title>`. The diff is exactly four scoped files and excludes every unrelated layout/card/table/source-pill change from stale PR #375. Focused dashboard tests pass 69/69; typecheck, lint (0 errors), diff check, and independent scope review pass. Preview, PR, merge, production deploy, live title verification, and superseded-PR closure remain pending.
