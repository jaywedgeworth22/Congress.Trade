...
## In Progress
- 2026-07-18 — CODEX — In Progress — **Production public UI navigation-only render incident** (branch `codex/live-render-fix`; isolated worktree `/Users/jay/.codex/worktrees/congress-live-render-fix`). Owner reports `https://congress.trade/` renders the navigation bar with no content below it while health/data APIs remain live. Reproducing in a real browser, tracing console/inline-script failure, adding a regression gate, then landing and production-verifying the smallest safe repair. KEEPOUT: public dashboard bootstrap/render slices in `app/src/ui/dashboardHtml.ts` and directly related smoke tests until closeout.
