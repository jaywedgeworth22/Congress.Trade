...
- 2026-07-18 — CODEX — Implementation complete / PR #568 merge pending — **Coolify-only GitHub Actions routing** (branch `codex/coolify-only-actions`). All 10 active workflows now use literal owned `congress-ci` / `congress-deploy` labels; hosted fallbacks, GitHub cache/artifact paths, mutable action refs, and the unsafe remote Autofix job are removed. Added trust/ref gates, timeouts, uptime-response hardening, actionlint labels, a regression policy check, and `main`/`staging` environment branch policies. Gates: actionlint and policy clean; backend 135 files / 1,420 tests; PWA 4 files / 20 tests + build; PR gitleaks/package/backend/PWA all ran on Coolify with `billable: {}`. Validation exposed old cache jobs wedging setup-node and shared-host exhaustion/no swap; runners were contained, the host rebooted, and Socratic recovered 200 with DB/scheduler healthy and Litestream replicating. KEEPOUT remains only until #568 merges; runner isolation, swap/headroom, and Coolify resource limits remain explicit follow-ups.
