...
- **[Congress.Trade][CLAUDE→AG] Benchmark model-catalog filtering: hide no-API-key models, drop `openrouter/auto`, add Opus 4.8 (2026-07-19) — INVESTIGATION DONE / CODE NOT STARTED / HANDED OFF TO AG.** Handoff note: `docs/handoffs/2026-07-19-claude-to-antigravity.md` (full task list, file:line map, verified facts). Owner ask from the admin Model Benchmarking panel: direct-provider rows he holds no key for (`openai:`/`gemini:`/`anthropic:`/`xai:`/`mistral:` — i.e. `LEGACY_CANDIDATES`) must disappear from the checkbox grid AND the A–E slot dropdowns; `openrouter/auto` out of benchmarking (trial later, keep one-line re-enable); add `anthropic/claude-opus-4.8` (slug verified LIVE vs the OpenRouter models API 2026-07-19 — text+image+**file**, 1M ctx, $5/$25 per M). Design: split the catalog — new `benchmarkSelectableCatalog()` (= `DEFAULT_CANDIDATES` + `LLAMAPARSE_CANDIDATES`) drives every UI surface, while `benchmarkModelCatalog()` keeps LEGACY + a new `NON_OFFERED_CANDIDATES` so decode/replay + historical config stay valid. ⚠️ `DEFAULT_CANDIDATES` is a **run set**, not just a menu (`admin/routes.ts:4290` — `POST /bake-off` defaults to it), so `auto` must be physically removed, not just hidden. ⚠️ **OPEN OWNER DECISION**: whether to pull `gpt-5.6-terra-pro` / `gpt-5.6-sol` from the offered set (my rec: remove both, keep catalog-valid — Terra Pro is a redundant high-cost twin of Terra; Sol keeps a niche as difficult-scan adjudicator). Overlap: PR #620 (`claude/resource-governors`) touches the same two files but at **disjoint** hunks (bakeoff ~1097-1365, routes ~3876-3937) — import-list adjacency only; whoever merges second rebases. Aligned with CODEX PR #556. ⛔ Owner constraint at handoff time: a benchmark run was live — **no merge/deploy/live-slot writes until it finishes**.
