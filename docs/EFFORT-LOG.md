...
## In Progress
- 2026-07-19 — CLAUDE — **Restore a GitHub-hosted CI escape hatch (`claude/ci-hosted-fallback`).** Congress.Trade's three required merge checks (`typecheck + test`, `PWA typecheck + test + build`, `gitleaks`) were hardcoded to `runs-on: [self-hosted, congress-ci]` by #568/`be1d4e1`, which removed the `CT_CI_RUNNER` variable switch originally added by #518. With no fallback, a degraded box blocks every merge repo-wide. That is exactly what happened today: PR #620's checks failed three times over ~40 minutes, never on code — every job died in `Set up job` on `Failed to download action .../actions/setup-node@49933ea... HttpClient.Timeout of 100 seconds` after 3 attempts, while `gitleaks` came back CANCELLED. The box itself was alive (Sentry CI Report succeeded on it mid-outage), so egress to codeload.github.com was degraded rather than down. PRs #637-#640 were queued behind the same wall. This restores `runs-on: ${{ (github.actor != 'dependabot[bot]' && vars.CT_CI_RUNNER) || 'ubuntu-latest' }}` in `ci.yml` (both jobs) + `security.yml`, so clearing/setting one repo variable switches the whole repo between hosted and self-hosted. Existing fork/Dependabot guards are untouched — Dependabot still never resolves to the persistent runner. **Operational note: `CT_CI_RUNNER` is currently UNSET (hosted, paid) to unblock the queue; set it back to `congress-ci` to return to $0 once the box's egress is verified healthy.**
