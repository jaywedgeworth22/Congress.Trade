...
## In Progress
- **Review Queue current drain + durable automation integration (CODEX, L) — BUILT + PREVIEWED; PRODUCTION PENDING APPROVAL 2026-07-11.** Owner-directed. Production remains 91 total / 27 pending / 64 resolved after three bounded dry-run-first House passes; all three source PDFs were visually verified and 36 rows corrected in place with stable IDs/cursors and audit receipts. Further production publishing remains paused because the deployed gate is underinclusive. Branch `codex/review-queue-resolution` integrates #257/#263 plus exact-material multiset agreement, distinct vendors, bounded retry/backoff/budget/leases, one-time legacy replay, monotonic review revisions, atomic human/normalizer/agreement row+filing+outbox transitions, durable holds/retry, live-only identity, coherent reviewer consensus, and exact `est_value` across all writers. Typecheck + 85 files / 764 tests pass; isolated preview version `8414f8c5-48cf-45b2-83d9-b5555b5f6bfc` is healthy at `https://congress-trade-preview.jaywedgeworth22.workers.dev` with migrations through `0034` and rendered QA complete. Nothing is pushed, merged, or production-deployed. KEEPOUT: preserve the dirty main checkout and MONET's worktree; production requires explicit owner approval.
