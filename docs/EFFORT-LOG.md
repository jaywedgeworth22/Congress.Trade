...
- 2026-07-18 — CODEX — Core merged as `2149d69` / PR #575 workspace follow-up pending — **Coolify-only GitHub Actions routing**. All 10 active workflows use literal owned `congress-ci` / `congress-deploy` labels; hosted fallbacks, GitHub cache/artifact paths, mutable action refs, and unsafe remote Autofix are removed. Added trust/ref gates, timeouts, uptime hardening, full-SHA enforcement, environment branch policies, and a regression policy. Gates: actionlint/policy clean; backend 135 files / 1,420 tests; PWA 4 files / 20 tests + build; PR gitleaks/package/backend/PWA ran on Coolify with `billable: {}`. Validation exposed old cache/sparse-workspace jobs and zero-swap host pressure; the host rebooted, Socratic recovered with DB/scheduler/Litestream healthy, and a persistent 4 GiB swapfile with swappiness 10 was added. PR #575 restores full workspaces and bans sparse checkout; #568's core routing is already merged. Dedicated runner isolation remains the explicit follow-up; no workflow KEEPOUT remains.
