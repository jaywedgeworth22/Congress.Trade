...
## In Progress
- **PWA post-merge review follow-ups (CODEX) — CORRECTIVE GATES GREEN / PR PENDING 2026-07-18.** Branch `codex/pwa-post-merge-followups` rebased onto exact `origin/main`; profile response fields now match the public client API, including null-safe count/volume formatting. Arbitrary dynamic `[ticker]` and `[slug]` routes were incompatible with the configured static export, so search now links to encoded `/asset?ticker=...` and `/politician?slug=...` pages that resolve identifiers client-side under Suspense without build-time live-data enumeration. PWA typecheck, 6 files / 30 tests, production static export, exported-page assertions, and diff check pass. Merge/deploy remains pending the corrective PR and green hosted checks; no production mutation. KEEPOUT remains until handoff.
...
- **Telemetry half-open single-probe circuit breaker hardening (CODEX + 3-agent review team, owner-directed, M) — IMPLEMENTED / PR #587 OPEN 2026-07-18.** Branch `codex/telemetry-half-open-lease`; carries forward the reviewed R2-only fallback fixes from superseded PR #584, ACKs Queue/DLQ only after exact-event R2 durability, quarantines terminal R2 poison objects instead of replaying them forever, bounds headers plus response-body parsing, fails closed on unavailable/malformed breaker control, persists a KV probe-in-flight gate behind atomic D1 lease 0046, and prevents stale contenders from reopening after a successful probe. Deterministic 400/409/413/422 legacy rows do not open the global circuit; after a bounded budget they are copied to an R2 quarantine prefix before D1 deletion, while transient/auth/rate-limit/5xx/timeout/malformed-2xx failures remain intact. Gates: 81 focused tests, serialized full 139 files / 1,463 tests, TypeScript/build, changed-file ESLint (0 errors), and whitespace checks pass. Review threads are now resolved, including the terminal R2 quarantine follow-up at `b7e4e54`; hosted checks rerunning. Production migration/deploy/live verification pending merge. KEEPOUT remains until deployment handoff.
