...
## In Progress
- **Auth cookie follow-up review fixes (CODEX) — IN PROGRESS 2026-07-18.** Branch `codex/auth-cookie-review-followup` from `origin/main`; changes scoped to `app/src/auth/session.ts`, `app/src/auth/routes.ts`, and focused auth tests. Focused auth tests 14/14 and app typecheck pass; no merge/deploy/thread resolution.
