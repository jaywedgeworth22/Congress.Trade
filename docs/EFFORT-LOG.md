...
## In Progress
- **PWA post-merge review follow-ups (CODEX) — IN PROGRESS 2026-07-18.** Branch `codex/pwa-post-merge-followups` from `origin/main`; backend summary-field mappings and Next 15 async route params are fixed, with focused formatter coverage. Typecheck/tests pass; build remains blocked by pre-existing static-export requirements on the newly merged dynamic routes. No merge/deploy/thread resolution. KEEPOUT: requested PWA profile/route files and focused PWA tests.
