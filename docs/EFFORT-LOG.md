...
- **Premium = delivery only: un-gate all feed columns (CLAUDE, S, owner-directed 2026-07-17) — PR OPEN (`claude/premium-delivery-only`).** Owner decision: "cap and country and no column should be premium — only delivery via webhook and SSE." Removed `tier: 'premium'` from the Cap/Country/Sector column defs, the locked-column chooser branch + "Premium enrichment" note, the `data-premium-col` upsell handler, the `columns` pricing intent, and the "Enrichment columns" line from pricing feature lists (`app/src/ui/dashboardHtml.ts` + test updates). Server side needed no change (fields were already returned to everyone). CSV full-history export left Premium (server-enforced 402 boundary, not a column) — flagged for owner if it should also be freed. Gates: typecheck + 132 files / 1,379 tests green. NOTE: overlaps AG's open PR #513 on `dashboardHtml.ts` (same column-def lines) — heads-up posted in #agent-sync; whichever merges second has a trivial rebase.
