...
## In Progress
- **Auth cookie follow-up review fixes (CODEX) — IN PROGRESS / PR #595 2026-07-18.** Branch `codex/auth-cookie-review-followup` from `origin/main`; changes scoped to `app/src/auth/session.ts`, `app/src/auth/routes.ts`, and focused auth tests. Auth tests 40/40 and app typecheck pass; pushed, not merged/deployed; no thread resolution.
