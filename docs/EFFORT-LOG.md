...
- **Switch to Cheapest OpenRouter Models (AG, S) — COMPLETED 2026-07-16.** Configured chamber-specific 3-model cascade matrices in wrangler.toml (House, Senate, and Exec) using different capability tiers based on file difficulty, leveraging the cheapest OpenRouter models, and deployed to production. Also switched AGREEMENT_AUTOPUBLISH_MODEL_A, B, and C to the cheapest capable OpenRouter models (gemini-2.5-flash-lite, qwen3-vl-8b-instruct, and nova-lite-v1).

- **Replace Sonnet Models with Sonnet 5 (AG, S) — COMPLETED 2026-07-16.** Removed old claude-sonnet-4-6 and claude-3-5-sonnet slugs across codebase and Infisical, replacing them with claude-sonnet-5 to take advantage of introductory pricing.