...
- 2026-07-16 — MONET — In Progress — **UW trial full-utilization** (owner-directed; $50/wk Unusual Whales API trial ends Jul 22; usage was 179/30,000 req/day on a single endpoint). Lane 1 (operator, no code): 90-day UW congress-trades sweep (date-walked `recent-trades` + `late-reports`) matched vs our `/api/transactions` — buckets: UW-only (candidate ingestion misses), we-only (coverage edge), matched-with-mismatches (amount-bracket + filed-date quality diffs); report → `/Users/jay/apps/research/2026-07-16-uw-90day-coverage-audit.{md,json}`. Lane 2 (`monet/uw-deep-match`): bounded date-targeted recent-trades lookups so pending disclosure-latency observations older than the ~200-row window can still match (52 stranded UW pendings in prod; monitor-first-seen lower-bound semantics; `UW_DEEP_MATCH_DATES_PER_RUN` knob; degrades silently post-trial). KEEPOUT: app/src/ingestion/fmpDisclosureLatency.ts + tests.
...
## In Progress
- **[MONET] Diagnosed prod webhook-delivery 401 wall + Review Queue triage (troubleshoot-issues-3c86a2 session, 2026-07-17) — DIAGNOSIS DONE, FIX LANDED IN Socratic.Trade (PR OPEN, not merged/deployed).** Owner shared an admin-dashboard screenshot showing repeating batches of 5 `HTTP 401` delivery failures plus one Review Queue item (`H-2026-20035003`, `bad_asset_name,low_confidence`). Root-caused the 401s to congress.trade's `app/src/delivery/webhook.ts` signing `X-Signature: sha256=<hex>` while Socratic.Trade's live receiver (`app/api/webhooks/congress/route.ts` via `src/lib/congress-webhook-auth.ts`) does a bare-hex exact-byte-length comparison with no prefix stripping — every signed delivery has been failing 401 (burning all 5 `MAX_ATTEMPTS`) since at least the 2026-07-12 cross-agent audit that first flagged it (never actually fixed — the shared package got a correct verifier, Socratic.Trade's route just never migrated onto it). No Congress.Trade code change needed (this is receiver-side); fixed + tested + PR opened in Socratic.Trade: https://github.com/jaywedgeworth22/Socratic.Trade/pull/1704 (branch `monet/fix-congress-webhook-signature-verify`, gate green: lint 0 errors/tsc clean/404 files-4701 tests/build clean). Separately confirmed the Review Queue item is expected behavior (validator correctly quarantined a filing with a garbled asset name for human review), not a bug — no action needed beyond normal manual resolution. Posted to #agent-sync.
