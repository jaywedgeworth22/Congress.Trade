...
## Deployed
- **Benchmark reuse/manual lineup/native-PDF verification + usage telemetry closeout (CODEX root, owner-directed 2026-07-15) — LOCAL IMPLEMENTED / PR PREP.** Isolated branch `codex/benchmark-reuse-clear` preserves branch histories, reuses prior successful same-chamber/doc/model cells before paid-call reservation, exposes run-all House/Senate/Executive, clear-history, and manual or benchmark-backed A/B/C save controls, and closes the telemetry Queue+R2 terminal gap with direct last-chance delivery plus D1 fallback/drain. Gates so far: benchmark/admin/UI focused 134/134, telemetry/readiness/migration focused 40/40, batch telemetry focused 56/56, TypeScript, diff-check, full suite 1,263/1,265 plus both timeout-only files clean on rerun (35/35). No paid provider canary, benchmark rerun, merge, deploy, production mutation, or secret output yet.
...
## Completed
- **Remove agreement model globals / require explicit chamber lineups (CODEX, S) — IN PROGRESS 2026-07-15.** Retiring the Wrangler/global A/B/C model values from code, examples, registry, and tests; agreement resolution now fails closed when a chamber's Infisical A/B/C lineup is incomplete. Existing House/Senate/Executive Infisical keys remain the sole model source.
...
## In Progress
- **Benchmark reuse/manual A-B-C/clear-history + final usage-telemetry D1 fallback (CODEX root, owner-directed 2026-07-15) — LOCAL FULL GATE GREEN / NOT PUSHED.** Isolated Codex branch `codex/benchmark-reuse-clear` in `/Users/jay/.codex/worktrees/congress-benchmark-reuse-clear` builds on current `origin/main` and avoids the dirty Antigravity checkout/PR #410 overlap. Benchmark history now supports chamber-scoped clear-history, manual A/B/C save without requiring a completed run, source-run-backed save with audit metadata, and successful-result reuse so already-successful model/doc cells are not rerun for paid calls. The prior final telemetry gap is closed with migration `0042_usage_telemetry_fallback_events`: Queue failure falls back to R2, then direct Usage Monitor delivery, then D1 retry outbox, with scheduled drain/readiness/migration parity coverage. Timing-sensitive test budgets were widened for two existing heavy/flaky tests without production SSE/bootstrap logic changes. Verification: `npm run typecheck` passed; focused telemetry/benchmark/schema/UI/admin routes passed 161/161; full app suite passed 127 files / 1,265 tests; `git diff --check` passed. No paid benchmark/provider call, Infisical write, production deploy, or Usage Monitor flag activation has run.
 - **Audit and land all verified local-only improvements; evaluate Batch savings and primary reader (CODEX + audit team, L) — PREVIEW VERIFIED / COMMITTING 2026-07-15 / KEEP OUT.** Integration branch `codex/local-production-sweep-20260714` in `/Users/jay/.codex/worktrees/congress-local-production-sweep` is based on fetched `origin/main@374c6c4`. The 32-worktree audit found one substantive local-only change: GPT-4o disclosure retirement `b9f5c72`; it is ported with the closed-PR #410 cache-shape, fail-soft, and benchmark-bypass fixes plus GPT-5.6 original-detail input. A no-generation OpenAI catalog check confirms Terra/Luna/Sol access. Live House settings currently resolve to `mistral:mistral-ocr-latest`; the malformed override is no longer active. Batch saves 50% for eligible asynchronous OpenAI work; the provider-neutral evaluation finds no measured corpus winner and provisionally recommends deterministic text first, then Mistral OCR 4 annotated, with cross-vendor escalation. TypeScript, ESLint quiet, and all 127 files / 1,274 tests pass. Isolated preview version `f1c4f3dd-36c7-4832-a1ce-146c0c9473f1` is healthy with Terra/Luna/Sol, no active GPT-4o UI choice, and admin fail-closed. Protected PR/merge, exact-main production deploy, and live verification remain. No uncommitted/secret-bearing files from dirty worktrees were staged; no paid model call ran.
 - **Benchmark reuse/manual A-B-C/clear-history + final usage-telemetry D1 fallback (CODEX root, owner-directed 2026-07-15) — LOCAL FULL GATE GREEN / NOT PUSHED.** Isolated Codex branch `codex/benchmark-reuse-clear` in `/Users/jay/.codex/worktrees/congress-benchmark-reuse-clear` builds on current `origin/main` and avoids the dirty Antigravity checkout/PR #410 overlap. Benchmark history now supports chamber-scoped clear-history, manual A/B/C save without requiring a completed run, source-run-backed save with audit metadata, and successful-result reuse so already-successful model/doc cells are not rerun for paid calls. The prior final telemetry gap is closed with migration `0042_usage_telemetry_fallback_events`: Queue failure falls back to R2, then direct Usage Monitor delivery, then D1 retry outbox, with scheduled drain/readiness/migration parity coverage. Timing-sensitive test budgets were widened for two existing heavy/flaky tests without production SSE/bootstrap logic changes. Verification: `npm run typecheck` passed; focused telemetry/benchmark/schema/UI/admin routes passed 161/161; full app suite passed 127 files / 1,265 tests; `git diff --check` passed. No paid benchmark/provider call, Infisical write, production deploy, or Usage Monitor flag activation has run.
- **Adopt immutable `congress-trading-shared` v1.7.1 in Congress.Trade (CODEX + peer reviewer, S) — LOCAL COMMIT + PREVIEW VERIFIED / READY / NOT PUSHED 2026-07-14.** Isolated worktree/branch `codex/shared-v171-consumer` from fetched `origin/main` `2435e75`; root and Worker manifests, lock resolutions, and `allowScripts` approvals exact-pin commit `0bc26ab9311a396f3f6b5cba0fb54fa7558a42b4`. Separate empty-cache installs contain JS/MJS/DTS/DMTS; CJS/ESM smokes, exact pin assertions, `npm ls`, and cross-app artifact hashes pass. Typecheck and the serialized full suite pass (127 files / 1,259 tests); the parent-directed isolated preview wrapper reran typecheck and all 127 files / 1,259 tests green before deploying Worker version `ed4189b2-4115-4779-ae4f-7781f3398b7d` to `https://congress-trade-preview.jaywedgeworth22.workers.dev`. Preview UI is HTTP 200, health reports `ok/db/schema=true` with `missing=[]`, and unauthenticated benchmark admin access fails closed with HTTP 401. Fresh baseline/current Wrangler dry-runs produce the byte-identical 6,187,729-byte runtime bundle (`d3c0be60...`), so no production runtime deploy is needed. Independent peer review found no P0/P1/P2; its sole P3 lightweight-tag wording issue is fixed. No push, PR, merge, provider call, production mutation, or production deploy has run.
...
- **Final Infisical bootstrap line-mapping repair (CODEX verifier/builder, S) — SCOPED REPAIR COMPLETE / REBASED / FULL GATE + FINAL REVIEW GREEN / READY FOR ROOT LANDING / NOT PUSHED 2026-07-14 on `codex/infisical-bootstrap-wiring`.** The dotenv 16.3.1 parser now locates the real key after leading whitespace/optional `export`, replaces the actual assignment once, and is byte-idempotent. Imported values round-trip losslessly or fail closed; provider keys stay excluded; live/broken symlinks and unsafe modes are rejected; output is `0600`. The final HOME-unset edge skips the optional global file instead of looking under cwd, with a hostile synthetic-secret regression. Rebased on `origin/main` `ae57bcc`; clean install found 0 vulnerabilities, typecheck passed, full Vitest 127 files / 1,258 tests, focused bootstrap 16/16, Node/Bash syntax and tracked/untracked diff checks passed. Independent final review reports LAND with no P0/P1/P2/P3. No real secret reads, remote config writes, provider calls, push, PR, merge, or deploy.
- **Fresh adversarial landing review of Infisical bootstrap wiring (CODEX-REVIEW, S) — REVIEW COMPLETE / LAND / NO FINDINGS 2026-07-14; READ-ONLY on `codex/infisical-bootstrap-wiring` vs `origin/main` `ae57bcc`.** P0/P1/P2/P3: none. Exact Wrangler dotenv grammar/decoding and key-line mapping, idempotency, complete-pair precedence, allowlisted inert global-file parsing, provider exclusion, mode `0600`, symlink rejection, test-control scrubbing, cloud setup, docs truth, unrelated-byte preservation, and the HOME-unset cwd hostile case pass. Independent focused 16/16 plus Node/Bash syntax and tracked/untracked diff checks pass; parent typecheck is green. No repo edits, real-secret reads, remote writes, provider calls, push, merge, or deploy by reviewer.
...
- **Usage telemetry stable-key replay hotfix (CODEX + verifier team, M) — PR #392 DEPLOYED / TERMINAL-FILE FOLLOW-UP PREVIEW GREEN / PR #394 OPEN 2026-07-14.** Merge `9be2115`; protected deploy run `29307602150`; Worker `97484c22-ee3d-431f-862a-924e52043ba8`; apex/workers readiness green after all 123 files / 1,150 tests, canonical `ship.sh`, and admin migrate.
  ...
  The stateful test DB now preserves real status transitions and exercises valid/invalid winner races, loser blocking, durability retries, cross-file counts/errors, exact stored identities, and legacy suppression. Fresh root gates pass focused 78/78, serialized full 123 files / 1,192 tests, typecheck, lint (0 errors; 109 inherited warnings), workflow YAML, shell parse, and diff check. Two final independent reviews each pass 74/74 plus typecheck/lint/diff and report no P0/P1/P2. Isolated preview version `359b12be-4a00-4923-97c0-b1f85400498a` passes `ok/db/schema=true`, `missing=[]`; unauthenticated batch GET/POST admin routes return 401. Ready PR #394 is open; hosted checks/review, protected merge/deploy, stale-job reconciliation, D1 receipt, and usage.jays.services closeout remain pending. Do not poll the production job again until this follow-up deploys.
- **Dashboard title-only stale-PR carry-forward (CODEX + verifier, S) — PR #390 / PREVIEW LIVE GREEN 2026-07-13.** On `codex/dashboard-title-only`, the production dashboard uses `<title>Congress.Trade</title>` and the historical mock uses `<title>Congress.Trade (Design Mock)</title>`. The diff is exactly four scoped files and excludes every unrelated layout/card/table/source-pill change from stale PR #375. Focused dashboard tests pass 69/69; typecheck, lint (0 errors), diff check, and independent scope review pass. The isolated preview wrapper passed 122 files / 1,129 tests, migrations/fixtures, deploy, and `ok/db/schema=true`, `missing=[]`; Worker version `b0e0ba1e-3d5e-4be5-a416-f7bd98597cac`; live preview DOM reports title `Congress.Trade`. Hosted gates, merge, production deploy, production title verification, and superseded-PR closure remain pending.
- **Benchmark daily-cap UTC-boundary hotfix (CODEX + verifier, S) — PREVIEW DEPLOYED / QA GREEN / PR PENDING 2026-07-13.** Late post-merge review of PR #385 found that an untouched paid benchmark cell reserved before UTC midnight could resume after midnight under the prior day's authorization. `codex/benchmark-cap-midnight-fix` binds authorization to the exact reservation UTC day, claims before supplemental reservation, token-fences claim release/preservation, propagates explicit resume consent in the UI, and tests day-keyed plus concurrent delivery behavior. Legacy profiles fail closed. Typecheck, focused 104/104, bounded full suite 122 files / 1,128 tests, lint (0 errors), diff check, and adversarial review pass. Exact branch preview `33923b98-14d5-4f30-a289-cc2054fc6b7b` passed migrations, fixtures, full tests, and `ok/db/schema=true`, `missing=[]`. Hosted/merge/production gates remain; no paid benchmark ran.
- **Persistent chamber benchmark history, measured cost/latency, per-branch A/B/C save controls, and complete third-party telemetry (CODEX + expert team, L) — PREVIEW DEPLOYED / QA GREEN / PR PENDING 2026-07-13.** Branch `codex/benchmark-history-actuals` is rebased on merged iOS-plan `main` (`83cc5094`) in an isolated Codex worktree. House/Senate/Executive runs and per-document/model results persist with measured latency, provider usage, spend provenance/coverage, prior-run comparison, and sequential cascade simulation; validated A/B/C lineups save through fenced/rollback-safe branch-specific Infisical writes. Paid runs require explicit confirmation, atomic daily caps, human-confirmed ground truth, and unknown-outcome retry confirmation. OpenAI 5.6 Terra/Luna/Sol Responses vision options are included with GPT-4o retained as a legacy control. All Worker and operator-script third-party HTTP calls are statically forced through secret-safe Usage Monitor telemetry with Queue-to-R2 fallback; simultaneous Queue+R2 failure remains the disclosed terminal gap. Local typecheck and lint pass (0 errors), bounded full suite passes 122 files / 1,124 tests, post-rebase focused suite passes 192/192, script telemetry passes 3/3, fresh migrations through `0040` plus synthetic three-chamber seed pass, and three expert audits report no P0/P1. Isolated preview `f54ea612-04cc-4795-b45b-12b176ce2627` is healthy at `https://congress-trade-preview.jaywedgeworth22.workers.dev`; D1 contains one saved run and three model results for every chamber, House exercises partial-cost coverage, latency ranges are populated, unauthenticated admin access returns 401, browser runtime has no console errors, and the exact build's open-admin local QA returns all histories/settings without provider calls or writes. PR #382 is superseded functionally and overlaps only two UI/routes files. No paid benchmark, production config mutation, or production deploy has run; next gates are PR/hosted CI, protected merge, then canonical production landing and live verification.
