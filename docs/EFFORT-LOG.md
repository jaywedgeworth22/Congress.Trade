...
## Deployed
- **LLM Payload Response Healing (AG) — OPEN via PR #514 (2026-07-17).** Adds `jsonrepair` as a deterministic fallback for malformed/truncated JSON in `visionUtils.ts`, moves shared constants from `visionLlm.ts`, and adds unit tests. **Not yet merged or deployed — this entry records the PR proposal, not a live deployment.**
