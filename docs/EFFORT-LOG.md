...
## In Progress
- **Usage telemetry stable-key replay hotfix (CODEX + verifier, S) — ADVERSARIAL PASS / PREVIEW + PR PENDING 2026-07-13.** Final branch tree `codex/usage-telemetry-idempotency` (based on `main@5da2f9c`) gives OpenAI, Anthropic, and Mistral provider-authored submission/terminal times precedence over late observation where persisted lifecycle data needs repair; CAS preserves valid concurrent winners and guarantees finite nonnegative turnaround. Submitted and returned document identities are trimmed, validated, and deduplicated; exact zero cost is preserved; stable measured-event identities are collision-resistant and replay-safe across result reordering. Invalid timestamps fail soft without losing product results. The sanctioned `batch-status` workflow prints only a bounded token-free summary and shares the non-cancelling `deploy-production` concurrency lock. Root verification passes focused 57/57, full 123 files / 1,150 tests, typecheck, lint (0 errors; 109 inherited warnings), workflow YAML/concurrency validation, and diff check. Independent final semantic review reports no P0/P1/P2 across timestamp attribution, CAS, stable keys, identity validation, zero cost, or workflow secret safety. No schema migration is required. Amend, push, isolated preview, PR, merge, deploy, and live reconciliation remain pending; production stale job `10611cb5-4e6e-4358-b638-4b530ae74c73` must be polled only after the corrected Worker deploys.
