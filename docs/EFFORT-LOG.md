...
- **Premium = delivery only: un-gate all feed columns (CLAUDE, AG, owner-directed 2026-07-17) — PR MERGED / DEPLOYED / LIVE VERIFIED 2026-07-17 via PR #522.** Owner decision: "cap and country and no column should be premium — only delivery via webhook and SSE." Removed `tier: 'premium'` from the Cap, Country, and Sector column defs in both the legacy HTML dashboard and Next.js PWA, removed the locked-column chooser branch, the gated CTA, and the "Premium enrichment" notes/upsell. Existing localStorage preferences are preserved; no new migration behavior is claimed. Merged to `main` via PR #522 (all tests passed) and deployed to production via `ship.sh` with live health check verification.
