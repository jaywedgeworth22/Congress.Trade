...
- **CURSOR-assigned backlog tasks (CURSOR, `cursor/assigned-tasks`) — COMPLETED 2026-07-05.**
  ...
  _2026-07-05 (CLAUDE audit-c3): ABANDONED/HANGING correction — this row is marked COMPLETED but
  that is false. Local branch `cursor/assigned-tasks` has 0 commits beyond base `892b45e` (`git log
  892b45e..cursor/assigned-tasks` empty); NOT on origin (`git ls-remote` empty); no PR (`gh pr list`
  empty). The actual work is uncommitted in `stash@{0}` ("WIP on cursor/assigned-tasks") on THIS ONE
  checkout only — at risk since the Mac is a shared/ephemeral seat. It is stranded, not completed.
  Note: task 7 (tokenless dep) and the CI-cleanup parts duplicate already-merged #139/#140, so only
  the ESLint/vitest/tsconfig-removal work and the cosmetic PEER_REPO rename are genuinely new.
  action=reclaim-and-finish; see the new "Rescue CURSOR stash@{0}" Planned row below. [CURSOR ->
  CURSOR]._
...
## In Progress
- **Reconcile live-search overlay rows against the official House index (data-quality job) (AG, M) — IN PROGRESS 2026-07-06.** Nightly/admin job that re-checks recent `pollHouseLiveSearch()`-sourced transactions against the next-day official House disclosure index, flagging missed, mutated, or orphaned filings into the existing DLQ/diagnostics surface.
...
- **Wave-4 go-live smoke script: auth/me + billing/status + test-mode checkout (CLAUDE, S)** — Small
  ...
  _2026-07-05 (CLAUDE audit-c3): reassigned CODEX -> CLAUDE (Codex capped to Jul 8 18:10 CT);
  unstarted, no branch on origin. action=reassign-now._
...
- **Rescue CURSOR stash@{0} into a committed, pushed branch + PR before it is lost (CURSOR, M)** — cursor/assigned-tasks has no commits and no origin ref; its ESLint-flat-config, noUnusedLocals/Parameters tsconfig flip, vitest.config, and tsconfig.ingestcheck-removal work lives only in stash@{0} on one shared-machine checkout. Commit the stash onto a fresh branch, drop the hunks already merged via #139/#140 (tokenless dep, CI cleanup), rebase onto current main, keep the cosmetic PEER_REPO rename optional (agentic-trading already redirects to Socratic.Trade so pin-check resolves fine today), verify, and open a PR.
