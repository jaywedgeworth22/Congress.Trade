...
- **Telemetry half-open single-probe circuit breaker hardening (CODEX, owner-directed, M) — IMPLEMENTED / PR #587 OPEN 2026-07-18.** Branch `codex/telemetry-half-open-lease`; carries forward the reviewed R2-only fallback fixes from superseded PR #584, ACKs Queue/DLQ only after exact-event R2 durability, bounds receiver requests, fails closed on unavailable or malformed breaker control state, and uses idempotent migration 0046 plus one atomic D1 singleton lease so only one isolate can perform a half-open probe. Gates: 74 focused telemetry/queue/readiness/migration tests, TypeScript/build, and changed-file ESLint pass; the parallel full suite reached 1,439/1,443 with four timeout-only cases, all 69 affected-file tests passing on serial rerun. Production migration/deploy/live verification pending PR checks and merge. KEEPOUT remains until merge/deploy handoff.
