...
- **Audit and Fix Sentry Issues, Shared Package, and LlamaParse Out-Of-Memory (AG, S) — COMPLETED 2026-07-12.** Fixed Dead Letter Queue infinite loop in `index.ts`, mocked premium user in tests for `FLEET-INFRA-35`, synced shared package pin for `FLEET-INFRA-34`, fixed duplicate-provider validation test. Wrote `scripts/retry-llamaparse-failed.mjs` and successfully re-extracted 15 failed Donald Trump 278-T documents using `llamaparse:fast`. Updated `ogeSource.ts` to include historic executive filers.
