...
## In Progress
- **Next.js PWA Dashboard Features Migration (AG) — IN PROGRESS (branch `antigravity/pwa-dashboard-migration`).** Porting column configuration, speed metrics provider scorecard, and trade table view features from the monolithic `dashboardHtml.ts` to modular React components in `clients/pwa/app/ui/` with type safety, client-side sorting, SWR fetching, and localStorage persistence. Verified all PWA tests pass (20 tests passed) and typecheck compiles cleanly.
- **Optimize visionLlm PDF Chunking (AG) — IN PROGRESS (branch `antigravity/optimize-pdf-chunking`).** Expanding `isMassiveContextModel` detection in `visionLlm.ts` to include all Gemini models, Claude 3+, GPT-4o+, and OpenRouter models. Raising the fallback chunk threshold from 15 pages to 50 pages for other models, maintaining raw bytes fallback on parse failure, and validating with unit tests.
