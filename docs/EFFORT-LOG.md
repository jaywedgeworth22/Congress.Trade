...
## In Progress
- 2026-07-18 — CODEX — Production repaired / `main` landing blocked — **Public UI navigation-only render incident** (branch `codex/live-render-fix`, PR #566, code commit `c2369bf`, Worker `8ff8c421-b19a-4cb6-82e9-eee59535d17d`). Root cause: `ba10898` deleted the Trades panel closing `</section>`, nesting every later view inside hidden `#view-feed`. Restored the close and added parsed-DOM sibling-panel coverage. Gates: UI 83/83, typecheck, serial full 135 files / 1,421 tests, independent adversarial review. Live browser now shows a 5,070px Trends body with real analytics, all five views are direct `main` children, Trades loads 50 rows/cards, Alerts and Trends switch correctly, console is clean, and health/schema are green. Canonical `ship.sh --deploy-only` was used because no migration changed and no local `ADMIN_TOKEN` was available. PR is mergeable but GitHub will not start required checks due account payment/spending-limit failure; admin merge is also refused. KEEPOUT remains until #566 lands on `main` so a later deploy cannot regress production.
- 2026-07-18 — CODEX — In Progress — **Coolify-only GitHub Actions routing** (branch `codex/coolify-only-actions`; worktree `/Users/jay/.codex/worktrees/congress-coolify-actions`). Owner clarified the Actions hard cap is intentionally exhausted and all repository workflows must use owned Coolify runners. Auditing every `runs-on`, routing CI/security to `congress-ci` and production rollout to `congress-deploy`, preserving trust guards, validating workflow syntax, then landing so blocked PR security checks can rerun without GitHub-hosted minutes. KEEPOUT: `.github/workflows/*` runner-selection changes until closeout.
