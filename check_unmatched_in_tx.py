...
print("=== CHECK DAN NEWHOUSE IN TRANSACTIONS ===")
...
WHERE fl.full_name LIKE '%Newhouse%' OR t.filer_id LIKE '%newhouse%'
LIMIT 20
"""))
for r in rows:
    print(r)

print("\n=== CHECK JAMES HIMES IN TRANSACTIONS ===")
rows = parse_html_table(run_sql("""
SELECT t.id, t.doc_id, t.filer_id, fl.full_name, t.ticker, t.tx_date, t.tx_type, f.filed_date
FROM transactions t
LEFT JOIN filers fl ON fl.bioguide_id = t.filer_id
LEFT JOIN filings f ON f.doc_id = t.doc_id
WHERE fl.full_name LIKE '%Himes%' OR t.filer_id LIKE '%himes%'
LIMIT 20
"""))
for r in rows:
    print(r)
