...
print("=== CHECK JAMES HIMES IN CANDIDATES ===")
rows = parse_html_table(run_sql("""
SELECT trade_hash, doc_id, provider, filer_name, ticker, tx_date, tx_type, status, match_method, provider_key
FROM trade_latency_candidates
WHERE filer_name LIKE '%Himes%' OR trade_hash LIKE '%himes%'
"""))
for r in rows:
    print(r)
