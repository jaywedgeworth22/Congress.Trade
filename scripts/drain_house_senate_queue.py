...
def extract_bracket_from_text(raw_text):
    ...
    if "$500,001 - $1,000,000" in s or "$500001 - $1000000" in s:
...
def snap_to_canonical_bracket(min_val, max_val, text=""):
    ...
    if len(nums) >= 2:
        val_min, val_max = int(nums[0]), int(nums[1])
        for b_min, b_max in CANONICAL_BRACKETS:
            if b_max is not None and abs(val_min - b_min) <= 100 and abs(val_max - b_max) <= 100:
                return b_min, b_max
...
def process_item(item):
    ...
    if not edits:
        try:
            ...
            print(f"[REJECTED] {doc_id} ({reason}) -> empty extraction / no valid transactions")
            return True
        except Exception as e:
            print(f"[FAIL-REJECT] {doc_id}: {e}")
            return False
    ...
    try:
        ...
        print(f"[FAIL] {doc_id}: {e.code} - {err_msg}")
        return False
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        print(f"Batch fetched: {len(items)} items. Total unresolved remaining in queue: {data.get('totals', {}).get('unresolved', 0)}")
        batch_processed = 0
        ...
        if batch_processed == 0:
            ...
            if consecutive_empty >= 3:
                print("3 consecutive empty batches, stopping zero-LLM drain loop.")
                break
            time.sleep(1)
        else:
            consecutive_empty = 0
    ...
    print(f"\n==========================================")
    print(f"Completed zero-LLM manual review queue drain loop. Total items processed: {total_processed}")
    print(f"==========================================")
