...
test("merge onto a multi-app remote keeps every sibling", () => {
  const dir = mkdtempSync(join(tmpdir(), "ct-ios-versions-"));
  ...
  assert.equal(merged.apps["trade.congress.ios"].marketingVersion, "1.0.81");
  assert.equal(merged.apps["trade.congress.ios"].build, "202608250100");
...
test("empty base is refused so a missing local file cannot wipe the fleet", () => {
  const dir = mkdtempSync(join(tmpdir(), "ct-ios-versions-empty-"));
