...
- Disclosure-latency race: `DISCLOSURE_LATENCY_WATCH_ENABLED`,
  ...
  **Yield-weighted spacing (all three):** 4 America/New_York bands
  ...
  (`FMP_LATENCY_API_KEY`, `RAPIDAPI_KEY` / `FMP_RAPIDAPI_KEY`, dual
  `UNUSUAL_WHALES_API_KEY` + `UNUSUALWHALES_API_KEY_2`, `QUIVER_API_KEY`) live
  under Infisical (shared project holds `RAPIDAPI_KEY`; CT app holds UW dual
  keys). FMP + UW dual-key rotation (`selectRotatedAvenue` / per-key budgets)
  is the pattern for multi-key sources. FMP is CT latency + Mac scout only.
