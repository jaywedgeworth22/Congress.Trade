 ...
 * the local-dev path.
 */
export const BENCHMARK_SCHEMA_STATEMENTS = [
  ...
  // Upgrade guards for an environment where the tables were created by an
  // earlier preview of this still-unreleased schema. The admin migrator treats
  // duplicate-column errors as already applied.
  `ALTER TABLE benchmark_runs ADD COLUMN request_profile_json TEXT NOT NULL DEFAULT '{}'`,
  `ALTER TABLE benchmark_model_results ADD COLUMN claim_token TEXT`,
  `ALTER TABLE benchmark_model_results ADD COLUMN lease_until TEXT`,
] as const;
