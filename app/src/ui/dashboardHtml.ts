...
@media (max-width: 720px), (orientation: landscape) and (max-width: 950px) and (max-height: 520px) {
  ...
  /* Eagle flight splash — curved path, bank, recede into header (not a mere scale) */
  #eagle-splash {
    position: fixed; inset: 0; z-index: 10000;
    background: radial-gradient(ellipse at 50% 42%, #0f1a30 0%, #08111f 62%, #050a12 100%);
    pointer-events: none;
    opacity: 1;
  ...
  #eagle-splash-mark {
    position: absolute;
    left: 0; top: 0;
    width: var(--eagle-fly-size, 220px);
    height: var(--eagle-fly-size, 220px);
    margin: 0;
    object-fit: contain;
  ...
  /* Soft ground shadow under the bird for depth while flying */
  #eagle-splash-shadow {
    position: absolute;
    left: 0; top: 0;
  ...
  @media (max-width: 520px) {
    #eagle-splash-mark { --eagle-fly-size: 170px; }
  }
...
<div id="eagle-splash" aria-hidden="true">
  <div id="eagle-splash-stage">
    <div id="eagle-splash-shadow"></div>
    <img id="eagle-splash-mark" src="/assets/eagle-splash.png" alt="" width="256" height="256" decoding="async" />
  </div>
</div>
<header class="top">
  <div class="brand" aria-label="Congress.Trade"><img class="brand-logo" id="brandLogo" src="/assets/brand-logo.png" data-src-dark="/assets/brand-logo-dark.png" data-src-light="/assets/brand-logo-light.png" alt="Congress.Trade" height="38" decoding="async" /></div>
...
/* ---- Eagle flight splash: curved flight path, bank, grow, recede into header ---- */
(function runEagleSplash() {
  var splash = document.getElementById('eagle-splash');
  var mark = document.getElementById('eagle-splash-mark');
  var shadow = document.getElementById('eagle-splash-shadow');
  var brand = document.getElementById('brandLogo') || document.querySelector('.brand-logo');
  ...
  function bez3d(a, b, c, d, t) {
    // derivative for tangent / banking
    var u = 1 - t;
  ...
  // Flight size envelope: start small (far), grow large (approach), shrink to nest (header)
  function sizeAt(t) {
    // t in 0..1 along whole flight
    if (t < 0.38) {
      // approach: 0.45 → 1.0
      var u = t / 0.38;
      return 0.42 + 0.58 * (1 - Math.pow(1 - u, 2.2));
    }
    // recede into header: 1.0 → endScale
    var v = (t - 0.38) / 0.62;
    v = easeInOut(Math.min(1, Math.max(0, v)));
    var end = window.__eagleEndScale || 0.16;
    return 1.0 + (end - 1.0) * v;
  }

  var flySize = parseFloat(getComputedStyle(mark).getPropertyValue('--eagle-fly-size')) || 220;
  if (window.innerWidth <= 520) flySize = 170;
  mark.style.width = flySize + 'px';
  mark.style.height = flySize + 'px';

  // Control points: enter from bottom-right (off-canvas), soar through center, exit to header
  var W = window.innerWidth;
  var H = window.innerHeight;
  var br = brand.getBoundingClientRect();
  var endSize = Math.max(28, Math.min(br.height || 38, 48));
  var endScale = endSize / flySize;
  window.__eagleEndScale = endScale;

  // Target: left side of lockup (eagle sits in mark)
  var nestX = br.left + Math.min(br.height * 0.55, br.width * 0.17);
  var nestY = br.top + br.height / 2;

  // Path: P0 off-screen BR → P1 mid-low bank → P2 center soar → P3 nest
  // Actually single cubic: start → pull → pull2 → end, with mid peak in center
  var p0 = { x: W * 1.12, y: H * 0.72 };
  var p1 = { x: W * 0.72, y: H * 0.22 };
  ...
  function frame(now) {
    ...
    // ease overall a bit so approach isn't linear
    var t = raw < 0.5
      ? 2 * raw * raw
      : 1 - Math.pow(-2 * raw + 2, 1.85) / 2;

    var x = bez3(p0.x, p1.x, p2.x, p3.x, t);
    var y = bez3(p0.y, p1.y, p2.y, p3.y, t);
    var dx = bez3d(p0.x, p1.x, p2.x, p3.x, t);
    var dy = bez3d(p0.y, p1.y, p2.y, p3.y, t);
    // Bank into turn (degrees)
    var bank = Math.atan2(dy, dx) * 180 / Math.PI;
    // Normalize bank relative to level flight toward left-up
    var rot = bank + 180; // face along path (image faces right-ish; adjust)
    // Eagle art faces right/down-right — align nose to velocity
    rot = bank + 12;
    // Wing beat: subtle pitch oscillation while flying (not at rest)
    var beat = Math.sin(raw * Math.PI * 7) * (1 - raw) * 7;
    var sc = sizeAt(raw);
    var blur = (raw > 0.15 && raw < 0.9) ? Math.min(1.6, Math.hypot(dx, dy) / 900) : 0;

    mark.style.transform =
      'translate(' + (x - flySize / 2) + 'px,' + (y - flySize / 2) + 'px) ' +
      'rotate(' + (rot + beat) + 'deg) scale(' + sc + ')';
    mark.style.filter =
      'drop-shadow(0 ' + (18 * sc) + 'px ' + (28 * sc) + 'px rgba(0,0,0,' + (0.5 * sc) + '))' +
      (blur > 0.2 ? ' blur(' + (blur * 0.35) + 'px)' : '');

    if (shadow) {
      var shY = y + flySize * 0.42 * sc;
      var shSc = 0.55 + 0.7 * sc;
      shadow.style.transform =
        'translate(' + (x - 60) + 'px,' + shY + 'px) scale(' + shSc + ', 0.55)';
      shadow.style.opacity = String(0.15 + 0.4 * sc * (1 - raw * 0.5));
    }

    if (raw < 1) {
      requestAnimationFrame(frame);
    } else {
      // Snap to nest, crossfade to brand lockup
  ...
  // Kick off on next frames so layout is ready
  requestAnimationFrame(function () {
    requestAnimationFrame(function (ts) { frame(ts); });
  });
})();
