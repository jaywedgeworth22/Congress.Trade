 ...
 */
/**
 * The provenance/confidence of the due_at timestamp.
 * - `system`: Event generated by our own system (e.g. ct_publish)
 * - `observed`: Captured within our polling window (bracketed)
 * - `claimed`: Provider reported this time, but we did not observe it
 * - `unbounded`: Backwards-compatible / undefined
 */
export type SnapshotConfidence = 'system' | 'observed' | 'claimed' | 'unbounded';
...
export function snapshotPlan(row: MatchRow): SnapshotPlanEntry[] {
  ...
  if (row.congress_first_seen_at) {
    // Our own publish clock — no bracketing question, always system.
    out.push({ event: 'ct_publish', dueAt: row.congress_first_seen_at, confidence: 'system', uncertaintySec: 0 });
  }
