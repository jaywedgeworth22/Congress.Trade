...
function stubAgree(text: string) {
  vi.stubGlobal('fetch', vi.fn(async (url: string) => {
    if (String(url).includes('api.openai.com')) return { ok: true, json: async () => ({ output_text: text }) } as unknown as Response;
...
describe('handleAgreementCheck (queue consumer)', () => {
  ...
  it('does not publish when a human resolves the review during model reads', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (url: string) => {
      ...
      if (String(url).includes('api.openai.com')) return { ok: true, json: async () => ({ output_text: ROW_AAPL }) } as unknown as Response;
