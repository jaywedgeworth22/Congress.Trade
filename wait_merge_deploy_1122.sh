#!/bin/bash
echo "Waiting for PR 1122 checks and merge..."
while true; do
  STATE=$(gh pr view 1122 --json state -q '.state')
  if [ "$STATE" = "MERGED" ]; then
    echo "PR 1122 is already merged."
    break
  fi
  gh pr merge 1122 --merge --admin
  if [ $? -eq 0 ]; then
    echo "Successfully merged PR 1122"
    break
  fi
  sleep 15
done
