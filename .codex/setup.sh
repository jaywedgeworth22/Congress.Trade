#!/usr/bin/env bash
# Codex Cloud setup for Congress.Trade. Secrets must be runtime environment variables
# when the agent phase needs them; setup-only secrets are removed before the agent runs.
set -euo pipefail
cd "$(dirname "$0")/.."

echo "==> Codex Cloud setup: Congress.Trade"
...
echo "Slack project: ${SLACK_PROJECT:-Congress.Trade}"
...
echo "Setup complete: Congress.Trade"
